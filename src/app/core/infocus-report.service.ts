...
export class InfocusReportService {
  ...
  getInfocusPDFbyId(id) {
    ...
    return this.http.get('http://localhost:8787/api/viewinfocuspdf/' + id.id + '.pdf', { headers });
  }

  getAllInfocusMeta() {
    return this.http.get('http://localhost:8787/api/getallinfocusmetalist');
  }

deleteInfocusReportById(id){
  return this.http.delete('http://localhost:8787/api/deleteinfocusreport/'+id)
}

publishPdfById(id, randomValue) {
  console.log(id)
  return this.http.get('http://localhost:8787/api/publishinfocus/?infocusmetaid=' + id+'&ver='+randomValue);
}
