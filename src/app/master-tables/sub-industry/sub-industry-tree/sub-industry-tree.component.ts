...
import {  of as ofObservable, BehaviorSubject, Observable } from 'rxjs';
import { AlertBoxComponent } from '../../../shared/alert-box/alert-box.component';
import { SelectionModel } from '@angular/cdk/collections';
import { SubIndustryTableAddComponent } from '../sub-industry-table-add/sub-industry-table-add.component';
...
@Component({
  selector: 'app-sub-industry-tree',
  templateUrl: './sub-industry-tree.component.html',
  styleUrls: ['./sub-industry-tree.component.css']
})
export class SubIndustryTreeComponent implements OnInit {
  @Input()
  TREEDATA = [];
...
/**
  ...
  addNewItem(node: TodoItemFlatNode) {
    ...
    let addItem = {};
    let industryIdList = [];
    industryIdList = parentNode.id;
    let industryId: number = industryIdList[0];
    // let industrySegmentId: number = industryIdList[industryIdList.length - 1];
    addItem['INDUSTRY ID'] = industryId;
    ...
    dialogEdit.afterClosed().subscribe(result => {
      ...
      let updatedNodeData: TodoItemNode = this.updateItem(
        addNodeData,
        result['INDUSTRY SUB SEGMENT']
      );
      result['children'] = addNodeData.children;
      result['item'] = result['INDUSTRY SUB SEGMENT'];
      if (result['item'] === '' || result['item'] === null) {
        ...
        let industryMarket = result['MARKET MAP'];
        industryMarket.item = result['INDUSTRY SUB SEGMENT'];
        result['MARKET MAP'] = industryMarket;
  ...
  editNode(node: TodoItemFlatNode) {
    ...
    let industryId: number = industryIdList[0];
    let industrySegmentId: number = industryIdList[industryIdList.length - 1];
    editValue['INDUSTRY ID'] = industryId;
