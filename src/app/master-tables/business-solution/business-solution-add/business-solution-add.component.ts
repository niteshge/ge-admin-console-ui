...
@Component({
  selector: 'app-business-solution-add',
  templateUrl: './business-solution-add.component.html',
  styleUrls: ['./business-solution-add.component.css']
})
export class BusinessSolutionAddComponent implements OnInit {
  rowData;
  ...
  constructor(public dialogRef: MatDialogRef<BusinessSolutionAddComponent>, private businessSolutionCoreServices: BusinessSolutionsService,
  ...
  techChange(key, value) {
    ...
    this.businessSolutionCoreServices.getTechnologiesSubSegment(this.rowData[key], randomValue)
      .subscribe(
        (response: Response) => {
  ...
  industryChange(key, value) {
    this.rowData[key] = value;
    let randomValue = Math.random() * 10;
    this.businessSolutionCoreServices.getIndustriesSubSegment(this.rowData[key], randomValue)
      .subscribe(
        (response: Response) => {
  ...
  techOrIndustryChange(key, value) {
    ...
    let randomValue = Math.random() * 10;
    if (value == "Industry") {
      this.businessSolutionCoreServices.getIndustries(randomValue)
        .subscribe(
          (response: Response) => {
      ...
      this.businessSolutionCoreServices.getTechnologies(randomValue)
        .subscribe(
          (response: Response) => {
            let listOfTech: any = [];
            listOfTech = response;
  ...
  onChangeName(value){
    ...
    let listOfSearchText:string[] = this.splitSearchText(this.rowData['SEARCH TEXT']);
    ...
    if(listOfSearchText.indexOf(previousNameValue)>-1){
      ...
      this.rowData['SEARCH TEXT']=this.concatenateSearchText(listOfSearchText);
  }
